...
const addToCart = async (req, res) => {
  try {
    ...
    const productData = await Products.findById({ _id: productId }).populate(
      'category',
    )
    ...
    const existProductIndex = existCartData.product.findIndex(
...
const quantityUpdate = async (req, res) => {
  try {
    ...
    const updateProduct = updateCartData.product.find(
      (pro) => pro.productId == proId,
    )
...
const loadCheckOut = async (req, res) => {
  try {
    ...
    const cartTotal = cartData.product.reduce(
    ...
    if (cartData.couponDiscount) {
      ...
      if (coupon) {
        couponDiscountAmount = Math.round(
          (coupon.discountAmount / 100) * cartTotal,
        )
      }
    }
