          ...
          if (couponData.userLimit <= 0) {
            ...
            if (couponData.expiryDate <= currentDate) {
              ...
              if (couponData.criteriaAmount >= cartTotal) {
                res.json({
                  cartAmount: true,
                  message: 'Your cart total does not meet the coupon criteria.',
                })
              } else {
                const discountPercentage = couponData.discountAmount
                const discountAmount = Math.round(
                  (discountPercentage / 100) * cartTotal,
                )
...
const removeCoupon = async (req, res) => {
  try {
    const couponId = req.body.couponId
    const userId = req.session.user_id
    const updateCoupo = await Coupon.findOneAndUpdate(
      { _id: couponId },
      { $pull: { usedUser: userId } },
    )
    const cartStatus = await Cart.findOneAndUpdate(
      { userId: userId },
