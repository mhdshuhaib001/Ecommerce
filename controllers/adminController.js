...
const loadDashboard = async (req, res) => {
  try {
    ...
    orderData.forEach((order) => {
      deliveredOrders += order.orderProducts.reduce(
        (acc, delivered) => (delivered.status === 'Delivered' ? acc + 1 : acc),
        0,
      )
      totalRevenue += order.orderProducts.reduce(
        (acc, product) =>
          product.status === 'Delivered' ? acc + product.totalPrice : acc,
        0,
      )
    ...
    const monthlyProductSales = await Order.aggregate([
      ...
      },
      {
        $group: {
          ...
          revenue: { $sum: '$orderProducts.totalPrice' },
        },
      },
    ])
    ...
    const paymentMethodDatas = await Order.aggregate([
      { $match: { 'orderProducts.status': 'Delivered' } },
...
const addCategory = async (req, res) => {
  try {
    ...
    const already = await categoryModel.findOne({
...
const salesReport = async (req, res) => {
  try {
    ...
    if (date && date !== 'undefined') {
      console.log('halloooooithe etheeekhn',);
      // Create a new Date object for each usage to avoid mutation
